"""
transaction 的業務邏輯層(service layer)：給 view、tasks、測試呼叫的同步業務邏輯。

放這裡的東西：跨 model 協調的業務邏輯(撮合、取消訂單)。
不放這裡的東西：
- Celery 進入點 → tasks.py(薄殼，只負責轉呼叫本模組)
- HTTP 層(權限、序列化)→ views.py
- 單一 model 自身的狀態操作 → models.py

全系統統一的鎖順序：TradingPair → Order → Wallet。
任何會 select_for_update 多種資源的函式都必須遵守，
順序不一致就有機會繞環造成 deadlock(見 test_concurrency.py)。
"""
